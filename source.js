...
async function hashPw(p) {
    ...
    const hash = await crypto.subtle.digest('SHA-256', enc.encode(p + 'cfnew_salt'));
    return btoa(String.fromCharCode(...new Uint8Array(hash)));
}

function renderLogin(isFarsi) {
    const t = isFarsi ? {
        title: 'ورود', password: 'رمز عبور', submit: 'ورود', wrong: 'رمز اشتباه است',
    } : {
        title: '面板登录', password: '面板密码', submit: '登录', wrong: '',
    };
    return `<!DOCTYPE html><html lang="${isFarsi ? 'fa' : 'zh'}"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>${t.title}</title><style>*{margin:0;padding:0;box-sizing:border-box}body{background:#0a0a0a;display:flex;align-items:center;justify-content:center;min-height:100vh;font-family:'Courier New',monospace}.box{background:rgba(0,20,0,0.6);border:2px solid #00ff00;padding:40px;text-align:center;max-width:400px;width:90%}.box h2{color:#00ff00;margin-bottom:20px;text-shadow:0 0 10px #00ff00}.box input{width:100%;padding:12px;background:rgba(0,0,0,0.8);border:2px solid #00ff00;color:#00ff00;font-family:'Courier New',monospace;font-size:16px;margin-bottom:15px;outline:none}.box input:focus{box-shadow:0 0 10px #00ff00}.box button{width:100%;padding:12px;background:rgba(0,255,0,0.15);border:2px solid #00ff00;color:#00ff00;font-size:16px;cursor:pointer;font-family:'Courier New',monospace}.box button:hover{background:rgba(0,255,0,0.3)}.err{color:#ff4444;margin-top:10px;display:none}</style></head><body><div class="box"><h2>${t.title}</h2><form id="f" method="post"><input type="password" id="p" name="pw" placeholder="${t.password}" autofocus><button type="submit">${t.submit}</button><p class="err" id="err">${t.wrong}</p></form></div><script>document.getElementById('f').addEventListener('submit',async function(e){e.preventDefault();const r=await fetch(window.location.pathname+'/login',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:'pw='+encodeURIComponent(document.getElementById('p').value)});if(r.ok){window.location.reload()}else{document.getElementById('err').style.display='block'}});</script></body></html>`;
}
...
export default {
    async fetch(request, env, ctx) {
        try {
            ...
            dp = getConfigValue('d', env.d || env.D) || 'dashboard';
            ...
            if (url.pathname.includes('/api/config')) {
                ...
                if (apiIndex > 0) {
                    ...
                    const cleanDp = dp.startsWith('/') ? dp.substring(1) : dp;
                    const isValid = (pathIdentifier === cleanDp);
            ...
            if (url.pathname.includes('/api/preferred-ips')) {
                ...
                if (apiIndex > 0) {
                    ...
                    let isValid = false;
                    const cleanDp = dp.startsWith('/') ? dp.substring(1) : dp;
                    isValid = (pathIdentifier === cleanDp);
    ...
    <body>
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                const cleanD = dp.startsWith('/') ? dp.substring(1) : dp;
                const normPath = url.pathname.endsWith('/') && url.pathname.length > 1 ? url.pathname.slice(0, -1) : url.pathname;
                ...
                // 订阅: /{st}/sub 优先，/{d}/sub 兜底
                if (normPath.endsWith('/sub')) {
                    const subToken = normPath.replace('/sub', '').substring(1);
                    const cleanSt = (st || '').startsWith('/') ? st.substring(1) : (st || '');
                    if ((cleanSt && subToken === cleanSt) || subToken === cleanD) {
    ...
    <body>
        ...
        <div class="container">
            ...
            <div class="card" id="configCard" style="display: none;">
                ...
                <div id="configContent" style="display: none;">
                    ...
                    <h3 style="color: #ff6600; margin: 20px 0 15px 0; font-size: 1.2rem;">🔒 ${isFarsi ? 'تنظیمات امنیتی' : '安全配置'}</h3>
                    <form id="securityConfigForm" style="margin-bottom: 20px; border: 2px solid #ff6600; padding: 15px; background: rgba(30, 10, 0, 0.4);">
                        <div style="margin-bottom: 15px;">
                            <label style="display: block; margin-bottom: 8px; color: #ffaa00; font-weight: bold;">${isFarsi ? 'مسیر داشبورد (d)' : '面板路径 (d)'}</label>
                            <input type="text" id="dpath" placeholder="${isFarsi ? 'پیش‌فرض: dashboard' : '默认: dashboard'}" style="width: 100%; padding: 12px; background: rgba(0, 0, 0, 0.8); border: 2px solid #ffaa00; color: #ffaa00; font-family: 'Courier New', monospace; font-size: 14px;">
                            <small style="color: #cc8800; font-size: 0.85rem;">${isFarsi ? 'مسیر دسترسی به داشبورد مدیریت' : '管理面板的访问路径，留空默认为 /dashboard'}</small>
                        </div>
                        <div style="margin-bottom: 15px;">
                            <label style="display: block; margin-bottom: 8px; color: #ffaa00; font-weight: bold;">${isFarsi ? 'رمز عبور داشبورد (pw)' : '面板密码 (pw)'}</label>
                            <input type="password" id="pwconfig" placeholder="${isFarsi ? 'رمز عبور برای داشبورد' : '设置面板登录密码'}" style="width: 100%; padding: 12px; background: rgba(0, 0, 0, 0.8); border: 2px solid #ffaa00; color: #ffaa00; font-family: 'Courier New', monospace; font-size: 14px;">
                            <small style="color: #cc8800; font-size: 0.85rem;">${isFarsi ? 'برای دسترسی به داشبورد نیاز به رمز عبور است' : '设置后访问面板需要输入密码，留空则不启用密码'}</small>
                        </div>
                        <div style="margin-bottom: 15px;">
                            <label style="display: block; margin-bottom: 8px; color: #ffaa00; font-weight: bold;">${isFarsi ? 'توکن اشتراک (st)' : '订阅凭证 (st)'}</label>
                            <div style="display: flex; gap: 10px;">
                                <input type="text" id="stconfig" placeholder="${isFarsi ? 'توکن اشتراک' : '留空则使用面板路径'}" style="flex: 1; padding: 12px; background: rgba(0, 0, 0, 0.8); border: 2px solid #ffaa00; color: #ffaa00; font-family: 'Courier New', monospace; font-size: 14px;" readonly>
                                <button type="button" onclick="regenerateSt()" style="background: rgba(255, 100, 0, 0.2); border: 2px solid #ff6600; padding: 12px 20px; color: #ff6600; font-family: 'Courier New', monospace; font-weight: bold; cursor: pointer; white-space: nowrap;">🔄 ${isFarsi ? 'تولید مجدد' : '重新生成'}</button>
                            </div>
                            <small id="stUrlDisplay" style="color: #ffaa00; font-size: 0.85rem; display: block; margin-top: 5px;"></small>
                        </div>
                        <button type="submit" style="background: rgba(255, 100, 0, 0.15); border: 2px solid #ff6600; padding: 12px 24px; color: #ff6600; font-family: 'Courier New', monospace; font-weight: bold; cursor: pointer;">${t.saveConfig}</button>
                    </form>
        ...
        <script>
            ...
            function generateClientLink(clientType, clientName) {
                ...
                var subscriptionUrl = SUB_TOKEN
                    ? (currentUrl.split('/')[0] + '//' + currentUrl.split('/')[2]) + '/' + SUB_TOKEN + '/sub'
                    : currentUrl + "/sub";
                var schemeUrl = '';
            ...
            async function loadCurrentConfig() {
                ...
                try {
                    ...
                    if (document.getElementById('dpath')) {
                        document.getElementById('dpath').value = config.d || '';
                    }
                    if (document.getElementById('pwconfig')) {
                        document.getElementById('pwconfig').value = config.pw || '';
                    }
                    if (document.getElementById('stconfig')) {
                        document.getElementById('stconfig').value = config.st || '';
                        if (config.st) updateStDisplay(config.st);
                    }
            ...
            function updatePathTypeStatus(cp) {
                ...
                const dpPath = (currentPath && currentPath !== '') ? currentPath : 'dashboard';
                const baseUrl = currentUrl.split('/')[0] + '//' + currentUrl.split('/')[2];
                var html = '<div style="color: #44ff44;">面板路径: <strong>' + dpPath + '</strong></div>';
                html += '<div style="margin-top: 5px; font-size: 0.9rem; color: #00aa00;">面板地址: ' + baseUrl + '/' + dpPath + '</div>';
                if (SUB_TOKEN) {
                    html += '<div style="margin-top: 5px; font-size: 0.9rem; color: #00ff00;">订阅地址: ' + baseUrl + '/' + SUB_TOKEN + '/sub</div>';
                }
                pathTypeStatus.innerHTML = html;
            }
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                if (securityConfigForm) {
                    securityConfigForm.addEventListener('submit', async function(e) {
                        e.preventDefault();
                        const configData = {
                            d: document.getElementById('dpath').value,
                            pw: document.getElementById('pwconfig').value,
                            st: document.getElementById('stconfig').value
                        };
                        await saveConfig(configData);
                        if (configData.st) updateStDisplay(configData.st);
                        // 更新主页面订阅链接变量
                        window.SUB_TOKEN = configData.st;
                        alert('安全配置已保存' + (configData.st ? '，订阅链接已更新' : ''));
                        window.location.reload();
                    });
                }

                // 重新生成订阅凭证
                window.regenerateSt = function() {
                    const chars = 'abcdefghijklmnopqrstuvwxyz0123456789';
                    var token = '';
                    for (var i = 0; i < 16; i++) {
                        token += chars[Math.floor(Math.random() * chars.length)];
                    }
                    document.getElementById('stconfig').value = token;
                    updateStDisplay(token);
                };

                // 更新订阅地址显示
                window.updateStDisplay = function(token) {
                    const el = document.getElementById('stUrlDisplay');
                    if (el && token) {
                        const baseUrl = window.location.origin;
                        el.textContent = '订阅地址: ' + baseUrl + '/' + token + '/sub';
                    }
                };

                // 初始化显示
                if (document.getElementById('stconfig') && document.getElementById('stconfig').value) {
                    updateStDisplay(document.getElementById('stconfig').value);
                }
                ...
                if (advancedConfigForm) {
                    advancedConfigForm.addEventListener('submit', async function(e) {
                        ...
                        const configData = {
                            ...
                            pw: document.getElementById('pwconfig').value,
                            st: document.getElementById('stconfig').value,
                            scu: document.getElementById('scu').value, ena: document.getElementById('ena').checked ? 'yes' : 'no', epd: document.getElementById('epd').checked ? 'yes' : 'no', epi: document.getElementById('epi').checked ? 'yes' : 'no', egi: document.getElementById('egi').checked ? 'yes' : 'no', ae: document.getElementById('apiEnabled').value,
